...
from src.PageObjects.Reports.RunReports.RunReportsMainView.MyReportPage import MyReportPage
from src.PageObjects.Reports.RunReports.RunReportsMainView.ReportPage import ReportPage


class Test_Report(BaseTest):
    @pytestrail.case('C32814')
    def test_ReportPage(self):
        ...
        time.sleep(2)
        tab_found = self.reportPage.validate_Report_type_tabs("My Reports")
        assert tab_found
        self.reportPage.click_report_type_tab("My Reports")
        self.myReportPage = MyReportPage(self.driver)
        page_found = self.myReportPage.validate_MyReportsPage()
        assert page_found
