...
class Test_AutomationRules(BaseTest):
    @pytestrail.case('C40100', 'C40104', 'C40105')
    def test_automationRule(self):
        ...
        flag = self.editAutomationRulePage.isSelected_Trigger_Action("Employee Email")
        assert bool(False) == flag
        self.editAutomationRulePage.click_on_SettingsTab("Employee Email Settings")
        time.sleep(2)
        assert self.editAutomationRulePage.isSelected_Trigger_Action("Employee Email")
        self.editAutomationRulePage.click_cancel()
        assert self.editAutomationRulePage.isAvailable_EditCancelAlert()
        self.editAutomationRulePage.click_Yes()
        time.sleep(2)
