...
class Test_LocateSchedule(BaseTest):
    ...
    def test_LocateScheduleView(self):
        ...
        assert flag
        actual = self.addEditTrackingSchedulePage.get_Help_tooltip()
        expected = "Location is tracked when the manager checks the location from the dashboard or when employee updates location from mobile app"
