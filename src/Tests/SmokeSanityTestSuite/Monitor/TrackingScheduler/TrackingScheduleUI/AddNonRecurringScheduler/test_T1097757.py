...
class Test_Groups(BaseTest):
    ...
    def test_GroupsUI(self):
        ...
        assert page_found
        timeZone = self.settingsPage.get_timeZone()

        print(" Actual timezone: "+timeZone)
        time.sleep(2)
        ...
        Schedule_timezone = self.addEditTrackingSchedulePage.get_timeZone()
        print("Expected:"+Schedule_timezone)
        assert timeZone == Schedule_timezone
