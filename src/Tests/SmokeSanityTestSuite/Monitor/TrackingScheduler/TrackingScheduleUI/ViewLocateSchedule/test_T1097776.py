...
class Test_LocateSchedule(BaseTest):
    @pytestrail.case('C32991', 'C32992', 'C32993', 'C32994', 'C32995', 'C32996', 'C32997', 'C32998')
    def test_LocateScheduleView(self):
        ...
        time.sleep(2)
        self.trackingSchedulerPage.click_view_icon_for_schedule(scheduleName)
        self.viewLocateSchedulePage = ViewLocateSchedulePage(self.driver)
        assert self.viewLocateSchedulePage.validateHeaderTitle()

        assert self.viewLocateSchedulePage.validate_edit_link()
        assert self.viewLocateSchedulePage.validate_delete_link()
        self.viewLocateSchedulePage.deleteSchedule()
        self.viewLocateSchedulePage.dismiss_popup()
        time.sleep(1)
        self.trackingSchedulerPage.click_view_icon_for_schedule(scheduleName)
        self.viewLocateSchedulePage.editSchedule()
        assert self.addEditTrackingSchedulePage.validate_EditTrackingScheduleModal()
        self.addEditTrackingSchedulePage.click_Save()
        time.sleep(1)
        self.trackingSchedulerPage.click_view_icon_for_schedule(scheduleName)
        self.viewLocateSchedulePage.click_Done()
        time.sleep(1)
        self.trackingSchedulerPage.click_view_icon_for_schedule(scheduleName)
        self.viewLocateSchedulePage.deleteSchedule()
