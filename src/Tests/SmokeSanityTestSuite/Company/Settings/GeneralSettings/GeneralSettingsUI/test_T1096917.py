...
class Test_GeneralSettings(BaseTest):
    ...
    def test_GeneralSettingsUI(self):
        ...
        assert page_found
        primaryEmail = self.settingsPage.get_primaryEmailContactValue()
        self.settingsPage.set_PrimaryEmail_AsNotificationEmail()
        time.sleep(5)
        notificationEmail = self.settingsPage.get_NotificationEmailValue()
        assert primaryEmail == notificationEmail
