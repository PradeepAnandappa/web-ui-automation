...
class Test_Jobsites(BaseTest):
    ...
    def test_jobsites(self):
        ...
        assert self.jobSitesPage.validate_popupMessage("Are you sure you want to delete this record")
        self.jobSitesPage.cancel_deleteAction()
        time.sleep(2)
        self.jobSitesPage.click_View_for_JobSiteName(siteName)
        self.viewJobSitePage = ViewJobSitePage(self.driver)
        assert self.viewJobSitePage.validateHeaderTitle()
        self.viewJobSitePage.deleteJobSite()
