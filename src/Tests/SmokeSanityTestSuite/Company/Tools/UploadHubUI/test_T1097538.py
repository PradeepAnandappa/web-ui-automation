...
class Test_JobSites(BaseTest):
    ...
    def test_uploadHub(self):
        ...
        time.sleep(2)
        file_path = str(
            Path.home()) + "\\PycharmProjects\\allGeo_webApp\\src\\Resources\\Earning_codefileWithFewInvalidColumn.csv"
        self.uploadHubPage.uploadCSVFile(file_path)
        time.sleep(2)
        self.uploadHubPage.click_Upload()
        assert self.uploadHubPage.validate_UnmatchedColumnPopupheader()
        assert self.uploadHubPage.validate_UnmatchedColumn("TestCode")
