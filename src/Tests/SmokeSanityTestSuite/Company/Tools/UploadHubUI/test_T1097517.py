...
class Test_JobSites(BaseTest):
    ...
    def test_addJobSite(self):
        ...
        time.sleep(2)
        file_path = str(Path.home()) + "\\PycharmProjects\\allGeo_webApp\\src\\Resources\\jobsitesWithDuplicateData.csv"
        self.uploadHubPage.uploadCSVFile(file_path)
        time.sleep(2)
        self.uploadHubPage.click_Upload()
        self.uploadHubPage.validate_SuccessUpdateMessage("successfully")
        time.sleep(2)
        self.uploadHubPage.click_company_icon("Jobsite")
        ...
        self.jobSitesPage.confirm_DeleteAction()
        self.jobSitesPage.deleteJobSiteByName("TestSite3")
        self.jobSitesPage.confirm_DeleteAction()
