...
class Test_JobSites(BaseTest):
    ...
    def test_addJobSite(self):
        ...
        time.sleep(2)
        self.uploadHubPage.click_DownloadTaskData()
        time.sleep(5)
        file_path = str(Path.home()) + "\\PycharmProjects\\allGeo_webApp\\src\\Resources\\Download\\task_details.xlsx"
        self.uploadHubPage.uploadCSVFile(file_path)
        time.sleep(2)
        self.uploadHubPage.click_Upload()
        assert self.uploadHubPage.validate_SuccessUpdateMessage("successfully")
