...
class Test_JobSites(BaseTest):
    ...
    def test_addJobSite(self):
        ...
        time.sleep(2)
        file_path = str(Path.home()) + "\\PycharmProjects\\allGeo_webApp\\src\\Resources\\EmptyEarning_codeFile.csv"
        self.uploadHubPage.uploadCSVFile(file_path)
        time.sleep(2)
        self.uploadHubPage.click_Upload()
        assert self.uploadHubPage.validate_ErrorMessage("The uploaded file is empty")
