...
class Test_JobSites(BaseTest):
    ...
    def test_addJobSite(self):
        ...
        file_path = str(Path.home()) + "\\PycharmProjects\\allGeo_webApp\\src\\Resources\\EmployeeDataWithoutName.csv"
        self.uploadHubPage.uploadCSVFile(file_path)
        time.sleep(2)
        self.uploadHubPage.click_Upload()
        assert self.uploadHubPage.validate_SuccessUpdateMessage("entries were not uploaded")
