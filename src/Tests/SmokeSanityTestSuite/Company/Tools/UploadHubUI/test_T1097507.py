...
class Test_JobSites(BaseTest):
    @pytestrail.case('C30396')
    def test_uploadHub(self):
        ...
        file_path = str(Path.home()) + "\\PycharmProjects\\allGeo_webApp\\src\\Resources\\employeeDataWithFewInvalidRecords.csv"
        self.uploadHubPage.uploadCSVFile(file_path)
        time.sleep(2)
        self.uploadHubPage.click_Upload()
        assert self.uploadHubPage.validate_SuccessUpdateMessage("2 entries were uploaded\nError:1 entries were not uploaded")
        time.sleep(2)
        self.uploadHubPage.click_company_icon("Employee")
        time.sleep(2)
        self.employeesPage.deleteEmployeeByName("TestUser1")
        self.employeesPage.deleteEmployeeByName("TestUser3")
